...
AddressController.prototype.balances = function (req, res) {
    this.node.getAddressesSummary(req.addrs, {}, function (err, summaries) {
        if (err) {
        ...
        for (var i = 0; i < summaries.length; i++) {
            ...
            balances.push({
                "address": summary["addrStr"],
                "balance": summary["balanceSat"],
                "unconfirmed_balance": summary["unconfirmedBalanceSat"],
...
AddressController.prototype.check = function(req, res, next, addresses) {
  ...
  for(var i = 0; i < addresses.length; i++) {
    try {
      var bech32 = bitcoin.address.fromBech32(addresses[i]);
    } catch (e) {
      try {
        var base58 = bitcoin.address.fromBase58Check(addresses[i]);
      } catch(e) {
        return self.common.handleErrors({
